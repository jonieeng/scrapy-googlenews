...
class gSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for articles in response.css('div.dbsr'):
            ...
            banned_list = ['Financial Times']
            excerpt = articles.css('div.XTjFC.WF4CUc::text').get()
            if excerpt in banned_list:
                chicken = "I love KFC"
                l.add_value('excerpt', chicken)
                yield l.load_item()
            else:
                excerpt_page = articles.css('a::attr(href)').get()
